...
module.exports = (app) ->
	...
	app.post '/auth/local',  (req,res) ->
		...
		query.fetch().then( (user) ->
			return res.send(400, error: "Incorrect email or password.")  unless user
			...
			return res.send(400, error: "Incorrect email or password.")  unless user.validPassword(password)
...
module.exports.acceptablePassword = acceptablePassword = (params, res) ->
	if params.password isnt params.password_confirmation
		...
		res.send 400,
			...
			message: "The password confirmation didn't match the password."
			errors:
