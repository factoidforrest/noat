	...
	if not key?
		...
		Token.forge(key: key, type:'login').fetch(withRelated: ['tokenable']).then (token) ->
			...
			if not token?
				...
				return res.send(401, {name: 'authErr', message: 'Token Invalid', token: 'Invalid'})
			else if token.expired(app.get('token_expiry')) 
				logger.info('token expired')
				return res.send(401, {name: 'authErr', message: 'Token Expired', token: 'Expired'})
			else
				...
				if user.get('active')
					...
					return res.send(403, {name: 'authErr', message: "Account disabled."})
