	...
	it 'should list two cards for the logged in user', (done) ->
		login {}, (session, token) ->
			...
			.end (err, res) ->
				...
				programId = res.body[0].program_id
				#console.log('got api logged in test response of:', res)
				done err

	it 'should filter card list by program id', (done) ->
		#this is a bit fragile because it depends on there being 2 cards with that id created in the previous tests
		login {}, (session, token) ->
			console.log('logged in, now attempting to list cards')
			session
			.post('/card/list')
	...
	it 'should list transactions', (done) ->
		login {}, (session, token) ->
			...
			send({
				...
				console.log('got transactions ', res.body)
				done(err)
	...
	it 'should remove a card', (done) ->
		login {}, (session, token) ->
			...
			send({
				...
				console.log('card remove response: ', res.body)
